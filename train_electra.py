...
- Uses a Generator-Discriminator approach (like GANs)
- Generator: Small masked language model that proposes replacements
- Discriminator: Detects which tokens were replaced (our classifier uses this)
- More efficient: Learns from ALL tokens, not just 15% masked ones
- Same compute as BERT but better performance
"""
...
def main():
    ...
    model_name = MODEL_CONFIGS["electra"]  # "google/electra-base-discriminator"
    ...
    # AutoModelForSequenceClassification adds a linear layer on top of ELECTRA
    model = AutoModelForSequenceClassification.from_pretrained(
        model_name,
        num_labels=NUM_CLASSES,
        use_safetensors=True  # Force safetensors format (avoids torch.load)
    )
    model.to(DEVICE)
    ...
    plot_confusion_matrix(
        ...
        history["best_preds"],
        "ELECTRA",
        save_path="outputs/electra_confusion_matrix.png"
    )
    ...
    print("Results saved to outputs/electra_results.json")
    
    print("\n" + "="*60)
    print("ELECTRA TRAINING COMPLETE!")
    print("="*60)
    print(f"Best model saved to: {WEIGHTS_DIR}/electra_best.pt")
